			...
			<Container>
				<div className="mx-auto max-w-2xl lg:mx-0">
					...
					<p className="mt-4 font-display text-2xl tracking-tight text-blue-900">
						Découvrez les personnes qui ont fait des présentations au Code @
						Québec.
					</p>
				</div>
				<TabGroup
					className="mt-14 grid grid-cols-1 items-start gap-x-8 gap-y-8 lg:gap-y-16 lg:mt-24 lg:grid-cols-4"
				...
				>
									...
									{events.map((event, eventIndex) => (
										<div key={event.date} className="relative lg:pl-8">
											...
											<div className="relative cursor-pointer">
												<div
													className={clsx(
														"p-2 pb-0 hover:bg-blue-50/50 rounded-2xl ",
					...
					<TabPanels className="lg:col-span-3">
						{events.map((event) => (
							<TabPanel
								key={event.date}
								className="gap-x-8 gap-y-6 justify-center not-lg:items-center flex flex-col data-selected:not-data-focus:outline-hidden sm:gap-y-12"
								unmount={false}
							>
										...
										<div className="w-80 flex-none mr-4">
											...
											<h3 className="mt-2 font-display flex items-center gap-2 text-xl font-bold tracking-tight text-slate-900">
												...
												{talk.profile.profileLinkedIn ? (
													<a
														className="text-gray-400 -mt-0.5 hover:text-gray-500 font-light"
														target="_blank"
														rel="noreferrer"
												...
												{talk.profile.company ? (
													talk.profile.companyLinkedIn ? (
														<a
															...
															href={talk.profile.companyLinkedIn}
														>
															{talk.profile.company}
														</a>
													) : (
