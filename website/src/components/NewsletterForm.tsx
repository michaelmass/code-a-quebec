...
export const NewsletterForm = () => {
  ...
  const hasRecentSubmission = () => {
    ...
    if (previousTimestamp && Number(previousTimestamp) + 180 * 1000 > timestamp) {
      setFormState(ERROR)
  ...
  const handleSubmit = (event: React.FormEvent<HTMLFormElement>) => {
    ...
    fetch(`https://${domain}/api/newsletter-form/${formId}`, {
      ...
      .then(async res => {
        ...
        if (!res.ok) {
          setFormState(ERROR)
          setErrorMessage("Il y a eu une erreur, assure-toi d'entrer un email valide")
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <div className="mt-5 flex rounded-3xl bg-white py-2.5 pr-2.5 shadow-xl shadow-blue-900/5 focus-within:ring-2 focus-within:ring-blue-900">
        {formState === SUCCESS ? (
          <p className="text-sm text-green-600 w-full text-center">Merci pour ton inscription !</p>
        ) : (
          <>
            <input
            ...
            />
            <Button type="submit" className="cursor-pointer" disabled={formState === SUBMITTING || formState === ERROR}>
      ...
      {formState === ERROR && <p className="mt-2 text-sm text-red-500 sm:text-base text-right">{errorMessage}</p>}
    </form>
  )
}
